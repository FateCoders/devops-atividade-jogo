...
func _physics_process(delta):
	...
	if nav_agent.get_target_position() != global_position:
		if global_position.distance_to(nav_agent.get_target_position()) < arrival_distance:
...
func _on_world_period_changed(period_name: String):
	# --- ADICIONADO: O ESPIÃO PRINCIPAL DO NPC ---
	print("--- NPC '", self.name, "' RECEBEU SINAL: '", period_name, "'. Meu estado ATUAL é: ", State.keys()[current_state], " ---")
	...
	if period_name == "NIGHT":
		...
		# --- ADICIONADO: ESPIÃO DENTRO DA CONDIÇÃO ---
		if current_state == State.EM_CASA:
			print(">>> CONDIÇÃO CUMPRIDA: Sou EM_CASA e é DIA. Devo sair agora!")
			_change_state(State.SAINDO_DE_CASA)
		else:
			print(">>> CONDIÇÃO FALHOU: É DIA, mas meu estado é '", State.keys()[current_state], "', então não farei nada.")
